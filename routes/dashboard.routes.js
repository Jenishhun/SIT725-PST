...
router.get('/api/balance-data', isAuth, async (req, res) => {
  try {
    const userId = new mongoose.Types.ObjectId(req.session.userId); // Fixed this line
    const period = req.query.period || 'month';
    const { startDate, endDate, format } = getDateRangeForPeriod(period);
...
router.get('/api/goals-progress', isAuth, async (req, res) => {
  try {
    ...
    const labels = goals.map(goal => `Goal (${goal.type} - ${goal.period})`);
    const currentValues = goals.map(goal => goal.amountLeft || 0);
    const targetValues = goals.map(goal => goal.amount || 0);
